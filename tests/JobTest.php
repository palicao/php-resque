...
use Resque\Tests\Fake\SomeJobClass;
use Resque\Tests\Fake\SomeStubFactory;
use Resque\Tests\Fake\TestJob;
use Resque\Tests\Fake\TestJobWithoutPerformMethod;
use Resque\Tests\Fake\TestJobWithSetUp;
use Resque\Tests\Fake\TestJobWithTearDown;
use Resque\Worker;
...
class JobTest extends TestCase
{
	...
	/**
	 * @expectedException \Resque\RedisException
	 */
	public function testRedisErrorThrowsExceptionOnJobCreation()
	{
		$mockCredis = $this->getMockBuilder(\Credis_Client::class)
		...
		Resque::setBackend(function($database) use ($mockCredis) {
			return new Redis('redis:6379', $database, $mockCredis);
		});
	...
	{
		$payload = array(
			'class' => SomeJobClass::class,
			'args' => null
		);
		$job = new Job('jobs', $payload);
		$instance = $job->getInstance();
		$this->assertInstanceOf(SomeJobClass::class, $instance);
	}
	...
	public function testUseFactoryToGetJobInstance()
	{
		...
		$instance = $job->getInstance();
		$this->assertInstanceOf(JobInterface::class, $instance);
	}
	...
	public function testDoNotUseFactoryToGetInstance()
	{
		...
		$testJob = $this->createMock(JobInterface::class);
